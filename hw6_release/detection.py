...
def hog_feature(image, pixel_per_cell=8):
    ...
    """
    ### YOUR CODE HERE
    hog_feature, hog_image = feature.hog(image, pixels_per_cell=(pixel_per_cell, pixel_per_cell), visualize=True, block_norm='L1')
    ### END YOUR CODE
...
def sliding_window(image, template_feature, step_size, window_size, pixel_per_cell=8,
    ...
    for r in range(0, H, step_size):
        for c in range(0, W, step_size):
            score = 0
            ### YOUR CODE HERE
            score = np.dot(template_feature, hog_feature(pad_image[r:r+winH,c:c+winW], pixel_per_cell=pixel_per_cell)[0])
            if score > max_score:
                max_score = score
                maxr = r
                maxc = c
...
def pyramid_score(image, template_feature, shape, step_size=20,
    ...
    ### YOUR CODE HERE
    for scale, img in images:
        score, r, c, response_map = sliding_window(image = img, 
...
def compute_displacement(part_centers, face_shape):
    ...
    ### YOUR CODE HERE
    main_center = face_shape[0] / 2, face_shape[1] / 2
...
def shift_heatmap(heatmap, mu):
    ...
    new_heatmap = interpolation.shift(heatmap, shift=mu)
    #new_heatmap = heatmap
    ### END YOUR CODE
    return new_heatmap
...
def gaussian_heatmap(heatmap_face, heatmaps, sigmas):
    ...
    for heatmap, sigma in zip(heatmaps, sigmas):
        heatmap_face += gaussian(heatmap, sigma)
    maxr, maxc = np.unravel_index(np.argmax(heatmap_face), heatmap_face.shape)
...
def detect_multiple(image, response_map):
    ...
    detected_faces = None
    ### YOUR CODE HERE
    print(response_map.max(), response_map.min())
    ### END YOUR CODE
