...
def harris_corners(img, window_size=3, k=0.04):
    ...
    window_ix2 = convolve(Ix2, window, mode='constant', cval=0)
    window_ixiy = convolve(IxIy, window, mode='constant', cval=0)
    window_iy2 = convolve(Iy2, window, mode='constant', cval=0)

    trace = window_ix2 + window_iy2
    response = window_ix2 * window_iy2 - window_ixiy * window_ixiy - k * trace * trace
    ### END YOUR CODE
...
def match_descriptors(desc1, desc2, threshold=0.5):
    ...
    for i in range(M):
        first_dist = dists[i][col_idxs[i]]
        second_dist = sorted_dists[i][1]
        assert np.isclose(first_dist, sorted_dists[i][0])
        if first_dist / second_dist < threshold:
            matches.append((i, col_idxs[i]))
...
def ransac(keypoints1, keypoints2, matches, n_iters=200, threshold=20):
    ...
    for _ in range(n_iters):
        samples = np.random.shuffle(matches)
        samples = matches[:n_samples]
        sample1 = pad(keypoints1[samples[:,0]])
        sample2 = pad(keypoints2[samples[:,1]])
        H = np.linalg.lstsq(sample2, sample1, rcond=None)[0]
        H[:,2] = np.array([0, 0, 1])
        diffs = matched2.dot(H) - matched1
        assert np.sum(diffs[:, 2]) == 0
        dists = np.sqrt(np.sum(diffs * diffs, axis=1))
        inliers_num = np.count_nonzero(dists <= threshold)
        #print(inliers_num)
        if inliers_num > n_inliers:
            n_inliers = inliers_num
            H_final = H.copy()
            max_inliers = dists <= threshold
    H = H_final
    ### END YOUR CODE
    ...
    for i in range(rows):
        for j in range(cols):
            ...
            for k in range(theta_block.shape[0]):
                ...
                cells[i][j][(int(theta_block[k] // degrees_per_bin)%9)] += G_block[k]
...
def linear_blend(img1_warped, img2_warped):
    ...
    img1_weight = np.zeros((out_H, out_W))
    img1_weight[:, :left_margin] = 1
    img1_weight[:, left_margin:right_margin] = np.linspace(1, 0, right_margin - left_margin)

    img2_weight = np.zeros((out_H, out_W))
    img2_weight[:, left_margin : right_margin] = np.linspace(0, 1, right_margin - left_margin)
    img2_weight[:, right_margin:] = 1
...
def stitch_multiple_images(imgs, desc_func=simple_descriptor, patch_size=5):
    ...
    for i in range(1, len(imgs)):
        H, _ = ransac(keypoints[i-1], keypoints[i], matches[i-1])
        output_shape, offset = get_output_space(panorama, [imgs[i]], [H])
        panorama_wraped = warp_image(panorama, np.eye(3), output_shape, offset)
        panorama_wraped[panorama_wraped == -1] = 0

        img_wraped = warp_image(imgs[i], H, output_shape, offset)
        img_wraped[img_wraped == -1] = 0

        panorama = linear_blend(panorama_wraped, img_wraped)
    ### END YOUR CODE
